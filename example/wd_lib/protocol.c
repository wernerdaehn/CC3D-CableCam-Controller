...
uint8_t configure_bt_module(Endpoints endpoint);
void wait_for_bt_module_response(uint8_t *btchar_string, uint8_t * btchar_string_length, Endpoints endpoint);
...
void evaluateCommand(Endpoints endpoint) {
	...
	case PROTOCOL_EEPROM_WRITE:
		...
		break;
	case PROTOCOL_DEBUG_LEVEL:
		if (argument_index == 1) {
			uint8_t p;
			if (stringToByte(arguments[0], &p)) {
				setDebugLevel(p, endpoint);
				writeProtocolHead(PROTOCOL_DEBUG_LEVEL, endpoint);
				writeProtocolOK(endpoint);
			}
		} else {
			writeProtocolHead(PROTOCOL_DEBUG_LEVEL, endpoint);
			writeProtocolInt(getDebugLevel(), endpoint);
			writeProtocolOK(endpoint);
		}
		break;
	case PROTOCOL_INPUT_CHANNELS:
		if (argument_index == 4) {
			...
			for (i=0; i<NO_CHANNELS; i++) {
				...
				writeProtocolInt(ICPWM_ReadDUTY(i), endpoint);
	...
	case PROTOCOL_HELP:
		printHelp(endpoint);
		break;
	case PROTOCOL_TIMING:
		{
			int16_t channel = 0;
			int16_t pos = 1;
			writeProtocolHead(PROTOCOL_TIMING, endpoint);
			for (channel = 0; channel < SUM_CHANNEL_START; channel++) {
				writeProtocolText("Channel ", endpoint);
				writeProtocolInt(channel, endpoint);
				writeProtocolText("=", endpoint);
				writeProtocolInt(getTimingTableValue(channel, 1), endpoint);
				for (pos = 1; pos < MAX_TIMING_SAMPLES; pos++) {
					writeProtocolText(" +", endpoint);
					int opos = pos -1;
					if (pos == 1) {
						opos = MAX_TIMING_SAMPLES-1;
					}
					writeProtocolInt(getTimingTableValue(channel, pos) - getTimingTableValue(channel, opos), endpoint);
				}
				writeProtocolText("\r\n", endpoint);
			}
			writeProtocolOK(endpoint);
	...
	case PROTOCOL_INPUT:
		{
			...
			for (channel = 0; channel < NO_CHANNELS; channel++) {
				...
				writeProtocolInt(channel, endpoint);
				writeProtocolText("] =", endpoint);
				writeProtocolInt(ICPWM_ReadDUTY(channel), endpoint);
				writeProtocolText(" -", endpoint);
				writeProtocolInt(ICPWM_ReadPause(channel), endpoint);
			}
			writeProtocolOK(endpoint);
		}
		break;
	...
	case PROTOCOL_BLUETOOTH:
		{
			if (endpoint == EndPoint_USB) {
				...
				if (configure_bt_module(endpoint) == 1) {
					writeProtocolOK(endpoint);
				} else {
...
void printHelp(Endpoints endpoint) {
	...
	PrintlnSerial_string("$d [<double>]                         set or print debug level 0 (nothing)...5 (everything)", endpoint);
...
uint8_t configure_bt_module(Endpoints endpoint) {
	...
	while (current_baud_rate_index < 7) {
		...
		PrintlnSerial(endpoint);
		PrintSerial_string("Testing Bluetooth module communication with ", endpoint);
		PrintSerial_int(baudrates[current_baud_rate_index], endpoint);
		PrintlnSerial_string(" baud.", endpoint);

		Uart_Init(baudrates[current_baud_rate_index]);
		PrintSerial_string("AT", EndPoint_UART3);
		wait_for_bt_module_response(btchar_string, &btchar_string_length, endpoint);
	...
	if (current_baud_rate_index != 7) {
		PrintlnSerial(endpoint);
		PrintSerial_string("Bluetooth module is currently configured for ", endpoint);
		PrintSerial_int(baudrates[current_baud_rate_index], endpoint);
		PrintlnSerial_string(" baud.", endpoint);

		if (current_baud_rate_index != 4) {
			PrintlnSerial_string("Sending command AT+BAUD6 to change it to 38400 baud", endpoint);
			...
			Uart_Init(38400);
			if (is_ok(btchar_string, &btchar_string_length)) {
				PrintlnSerial_string("Baud rate changed to 38400.", endpoint);
			} else {
				config_error = 1;
			}
		}
		if (config_error == 0) {
			PrintlnSerial(endpoint);
			PrintlnSerial_string("Setting name to CableCam using the command AT+NAMECableCam", endpoint);

			PrintSerial_string("AT+NAMECableCam", EndPoint_UART3);
			wait_for_bt_module_response(btchar_string, &btchar_string_length, endpoint);
			if (is_ok(btchar_string, &btchar_string_length)) {
				PrintlnSerial_string("Bluetooth Device Name is CableCam.", endpoint);
				PrintlnSerial(endpoint);
				PrintlnSerial_string("Setting pairing secure pin to 1234 using the command AT+PIN1234", endpoint);

				PrintSerial_string("AT+PIN1234", EndPoint_UART3);
				wait_for_bt_module_response(btchar_string, &btchar_string_length, endpoint);
				if (is_ok(btchar_string, &btchar_string_length)) {
					PrintlnSerial_string("Bluetooth Device configured successfully", endpoint);
					return 1;
				}
			}
		}
	}
	PrintlnSerial_string("Command not correctly processed, no OK response", endpoint);
	return 0;
}
...
void wait_for_bt_module_response(uint8_t *btchar_string, uint8_t * btchar_string_length, Endpoints endpoint) {
	...
	*btchar_string_length = 0;
	while (HAL_GetTick() - tick < 3000) {
		btchar = HAL_UART_Char_Getter(&Uart3Handle);
		if (btchar != 0) {
