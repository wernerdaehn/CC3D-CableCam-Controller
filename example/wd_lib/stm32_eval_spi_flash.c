...
void sFLASH_EraseSector(uint32_t SectorAddr) {
	...
	switch(HAL_SPI_Transmit(&SpiHandle, &command[0], 4, 5000))
	{
	case HAL_TIMEOUT:
...
void sFLASH_EraseBulk(void) {
	...
	switch(HAL_SPI_Transmit(&SpiHandle, &command[0], 1, 5000))
	{
	case HAL_TIMEOUT:
		/* A Timeout Occur ______________________________________________________*/
		/* Call Timeout Handler */
...
void sFLASH_WritePage(uint8_t* pBuffer, uint32_t WriteAddr,	uint16_t NumByteToWrite) {
	...
	switch(HAL_SPI_Transmit(&SpiHandle, pBuffer, NumByteToWrite, 5000))
	{
	case HAL_TIMEOUT:
		/* A Timeout Occur ______________________________________________________*/
		/* Call Timeout Handler */
...
uint32_t sFLASH_VerifyWrite(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead) {
	...
	while (NumByteToRead--) /*!< while there is data to be read */
	{
		...
		switch(HAL_SPI_TransmitReceive(&SpiHandle, &command[0], &byteinflash, 1, 5000))
		{
		case HAL_TIMEOUT:
			/* A Timeout Occur ______________________________________________________*/
			/* Call Timeout Handler */
...
void sFLASH_ReadBuffer(uint8_t* pBuffer, uint32_t ReadAddr,	uint32_t NumByteToRead) {
	...
	{
		/*!< Read a byte from the FLASH */
		switch(HAL_SPI_TransmitReceive(&SpiHandle, &command[0], pBuffer, 1, 5000))
		{
		case HAL_TIMEOUT:
			/* A Timeout Occur ______________________________________________________*/
			/* Call Timeout Handler */
...
uint32_t sFLASH_ReadID(void) {
	...
	/*!< Read a byte from the FLASH */
	switch(HAL_SPI_TransmitReceive(&SpiHandle, &command[0], &idsequence[0], 3, 5000))
	{
	case HAL_TIMEOUT:
		/* A Timeout Occur ______________________________________________________*/
		/* Call Timeout Handler */
...
void sFLASH_WaitForWriteEnd(void) {
	...
	do {
		...
		switch(HAL_SPI_TransmitReceive(&SpiHandle, &command[0], &flashstatus, 1, 5000))
		{
		case HAL_TIMEOUT:
			/* A Timeout Occur ______________________________________________________*/
			/* Call Timeout Handler */
